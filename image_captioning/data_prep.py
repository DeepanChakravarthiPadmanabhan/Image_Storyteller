...
from constants import annotation_folder, image_folder, top_k, \
    BUFFER_SIZE, img_name_val_file, cap_val_file, img_name_train_file, cap_train_file
from util import get_architecture, calc_max_length, load_image, map_func

class DataLoader():
    def __init__(self, annotation_folder, image_folder, num_examples):
        self.annotation_file = annotation_folder + 'captions_train2014.json' 
    ...
    def get_imgnames_captions(self):
        ...
        self.train_captions, self.img_name_vector = shuffle(all_captions, all_img_name_vector,
                                                  random_state=1)
        self.train_captions = self.train_captions[:self.num_examples]
        self.img_name_vector = self.img_name_vector[:self.num_examples]
    ...
    def get_dataset(self, architecture, BATCH_SIZE):

        # Get unique images.
        encode_train = sorted(set(self.img_name_vector))
        # Get architecture pre-trained
        ...
        dataset = dataset.shuffle(BUFFER_SIZE).padded_batch(BATCH_SIZE, padded_shapes=([None, 2048],[None]))
        ...
        dataset_val = dataset_val.shuffle(BUFFER_SIZE).padded_batch(BATCH_SIZE,
                                                                    padded_shapes=([None, 2048], [None]))
        dataset_val = dataset_val.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)

        with open(img_name_val_file + '.pkl', 'wb') as fp:
            pickle.dump(self.img_name_val, fp)

        with open(cap_val_file +'.pkl', 'wb') as fp:
            pickle.dump(self.cap_val, fp)

        with open(img_name_train_file + '.pkl', 'wb') as fp:
            pickle.dump(self.img_name_train, fp)

        with open(cap_train_file + '.pkl', 'wb') as fp:
